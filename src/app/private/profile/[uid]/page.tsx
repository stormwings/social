...
import { BuyKeyModal } from "@/components/smart/modals/BuyKeyModal";
import { SellKeyModal } from "@/components/smart/modals/SellKeyModal";
...
export default function UserProfile({ params }: { params: { uid: string } }) {
  ...
  const posts = useUserPosts({ uid: params.uid });
        ...
        <div className="bg-white text-black">
          ...
          <div className="pt-4 border-t">
            {keyCount && keyCount > 0 && posts && (
              <AdminPostFeed posts={posts} isOwner={false} />
            )}
          </div>
        </div>
        {userEthereumAddress && (
