...
export default function CommentSection({ postId, userAddress, user }: CommentSectionProps) {
  ...
  return (
    <>
      <div className="border-t" data-testid="add-comment-section">
        <AddComment userAddress={userAddress} onConfirm={sendComment} isEmpty={comments?.length === 0} />
      </div>
      ...
      >
        <div className="flex items-center space-x-2">
          <input
            ...
            type="text"
            value={text}
            placeholder={isEmpty ? "Be the first to comment" : "Add a comment..."}
            onChange={(e) => setText(e.target.value)}
            data-testid="comment-input"
          />
          <button type="submit" className="text-gray-500 hover:text-gray-700">
            <ChatBubbleLeftIcon className="h-6 w-6" data-testid="send-comment-button" />
          </button>
        </div>
      </form>
    </div>
  ...
  return (
    <>
      {comments.map((comment, i) => {
        ...
        return (
          <div key={i} className="px-4 py-3 flex flex-col space-x-2" data-testid={`comment-${i}`}>
            <div className="flex items-center space-x-2">
              <ProfilePicture userAddress={comment?.userAddress} />
              <div>
                <h4 className="font-bold text-sm" data-testid="comment-username">
                  {user?.username || "Anonymous"}
                </h4>
                <p className="text-xs text-gray-500" data-testid="comment-date">
                  {comment?.timestamp?.seconds ? convertTimestampToDate(comment.timestamp.seconds) : "Just now"}
                </p>
              </div>
            </div>
